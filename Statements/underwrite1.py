...
def pixel(tracking_id: str):
    ...
    try:
        ...
        # Do not break the pixel response to avoid showing broken image icons
...
def statements_and_match():
    analysis = None  # default so JSON always has the key
    try:
        ...
        # ---------- AI underwriting (Claude) ----------
        try:
            # only use real files, not storage:// pointers
            pdf_paths = [
                f["path"]
...
def _build_tracking_pixel_url(
    ...
    """
    Build an absolute URL to the tracking pixel endpoint.
    Includes deal_id and lender_name so we can aggregate per deal/funder.
    """
    from flask import url_for, request

    # Try to build a fully-qualified URL via url_for
    try:
        pixel_url = url_for(
            "underwrite.pixel",  # blueprint name = "underwrite"
            tracking_id=tracking_id,
            _external=True,
            deal_id=deal_id,
            lender=lender_name,
        ...
        # Fallback if url_for/_external is not available
        root = (request.url_root or "").rstrip("/")
        pixel_url = (
            f"{root}/underwrite/pixel/{tracking_id}.gif"
            f"?deal_id={deal_id}&lender={quote_plus(lender_name or '')}"
        )
    return pixel_url
...
def _inject_tracking_pixel(
    ...
    *,
    ...
    """
    pixel_url = _build_tracking_pixel_url(tracking_id, deal_id, lender_name)
    pixel_tag = (
        f'<img src="{html.escape(pixel_url)}" alt="" '
...
def send_emails():
    ...
    try:
        ...
        for lender in selected:
            ...
            if prov == "gmail":
                ok, provider_id = gmail_send(
                    token, final_subject, body_with_pixel,
                    sender_email, to_email, cc_list,
                    attachments=attachments_for_this,
                )  # gmail_send returns (bool, id_or_error_str) :contentReference[oaicite:0]{index=0}
            elif prov in ("outlook", "graph"):
                ok, provider_id = graph_send(
                    token, final_subject, body_with_pixel,
                    sender_email, to_email, cc_list,
                    attachments=attachments_for_this,
                )
...
def _record_open_event_for_tracking_id(tracking_id: str) -> None:
    ...
    """
    try:
        # 1) Find the matching doc_tracking row
        resp = (
        ...
        rows = resp.data or []
        if not rows:
            # No tracking row found; nothing to record
            return

        doc_tracking_id = rows[0]["id"]

        # 2) Build the open-event payload
        ...
        # We never want this to throw back to the client
...
def tracking_pixel(tracking_id: str):
    ...
    try:
        ...
        # Never break the pixel, even if logging fails
...
@bp.post("/wrap, methods=["POST"]")
@global_auth_before_request.exempt
def api_wrap():
...
def pdf_stats(token: str):
    try:
        ...
        return jsonify({
            ...
            "recent_views": views.data[:10]  # Last 10 views
        })
...
def api_wrap():
    ...
    # Create secure link instead of returning PDF
...
@bp.post("/wrap-secure, methods=["POST"]")
@bp.post("/wrap-secure/, methods=["POST"]")
def api_wrap_secure():
   ...
   # Send HTML email with inline PDF preview (reuses your send_email())
